function [experimentRecord timeStamps] = unmaskedPrimeTabletMain (experimentParameters, splashParameters, trialParameters)
% function [experimentRecord timeStamps] = unmaskedPrimeTabletMain (experimentParameters, splashParameters, trialParameters)
%
% Handles all the setting up for an unmasked prime study with the tablet.
% 
...
%Error handling
if nargin ~=3
    error(['Not enough arguments. Usage: [experimentRecord timeStamps] = unmaskedPrimeTabletMain ', ...
        '(experimentParameters, trialParameters). (unmaskedPrimeTabletMain.m)']);
end
...
try
        ...
        for trialInBlock = 1:nTrialsPerBlock
            ...
            while 1
                ...
                if isempty(pkt) %ie queue empty
                    break; %Jump to the next run of 'while 1' loop, ie poll the tablet again
                else
    ...
    if ~isequal(subID, 'practice')
        ...
        if ~exist(fileName, 'file')
            ...
            analyseActionData(subID, 'unmasked');
        else
            %Note this check is only done once - rename kghrhtgkrehk.xls immediately or it will be overwritten
            save kghrhtgkrehk_unmaskedPrimeTabletData experimentRecord timeStamps earlyKinematics lateKinematics experimentParameters trialParameters;
            analyseActionData('kghrhtgkrehk', 'unmasked');
            disp(['Warning! ', fileName, ' already existed (unmaskedPrimeTabletMain.m).']);
    ...
    cleanup;
    eval(['save ', subID,...
        '_unmaskedPrimeTabletData experimentRecord timeStamps earlyKinematics lateKinematics experimentParameters trialParameters']);
end
return
